...
```admonish info
Honest nodes only ever vote for confirmed snapshots, that is, prefixes of their best $\Pi_{\mathrm{lc}}$ chain truncated by the confirmation depth $\sigma$. Obviously the whole point of having the BFT protocol is that chain forks longer than $\sigma$ **can** occur in $\Pi_{\mathrm{lc}}$ &mdash; otherwise we'd just use $\Pi_{\mathrm{lc}}$ and have done. So it is not that we expect this case to be common, but if it happens then it will never fix itself: the consensus chain in $\Pi_{\mathrm{lc}}$ will continue on without ever including the transactions from $\mathsf{LOG}_{\mathrm{fin}}^t$ that were obtained from a snapshot of another fork.
```
